  ...
  init {
    ...
    testLaws(
      PrismLaws.laws(
        ...
        aGen = Arb.tuple2(genSum, Arb.int()),
        bGen = Arb.tuple2(Arb.string(), Arb.int()),
        funcGen = Arb.functionAToB(Arb.tuple2(Arb.string(), Arb.int())),
        EQA = Eq.any(),
        EQOptionB = Eq.any()
      )
    )
    ...
    testLaws(
      PrismLaws.laws(
        ...
        aGen = Arb.tuple2(Arb.int(), genSum),
        bGen = Arb.tuple2(Arb.int(), Arb.string()),
        funcGen = Arb.functionAToB(Arb.tuple2(Arb.int(), Arb.string())),
        EQA = Eq.any(),
        EQOptionB = Eq.any()
      )
    )
    ...
    "Checking if a prism exists with a target" {
      forAll(genSum, genSum, Arb.bool()) { a, other, bool ->
    ...
    "Setting a target on a prism should set the correct target" {
      forAll(genSumTypeA, Arb.string()) { a, string ->
    ...
    "Finding a target using a predicate within a Lens should be wrapped in the correct option result" {
      forAll(genSum, Arb.bool()) { sum, predicate ->
